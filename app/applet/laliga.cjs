...
https.get(options, res => {
  ...
  res.on('end', () => {
    ...
    const laligaMatch = text.match(/La Liga \[ edit \](.*?)(?:Segunda División|Serie A) \[ edit \]/);
    if (!laligaMatch) {
    ...
    for (let i = 0; i < teamSections.length; i += 2) {
      ...
      if (inMatch) {
        const players = inMatch.matchAll(/(GK|DF|MF|FW)\s+[A-Z]{3}\s+([A-Za-z \-áéíóúüñćč]+?)\s+\(/g);
      ...
      if (outMatch) {
        const players = outMatch.matchAll(/(GK|DF|MF|FW)\s+[A-Z]{3}\s+([A-Za-z \-áéíóúüñćč]+?)\s+\(/g);
        for (const p of players) {
          const pos = p[1];
          const name = p[2].trim();
    ...
    fs.writeFileSync('/app/applet/src/data/leagues.ts', newLines.join('\n'));
